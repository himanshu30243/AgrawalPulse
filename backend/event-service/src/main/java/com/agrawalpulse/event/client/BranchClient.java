...
// Same pattern as family-service's/membership-service's own BranchClient (chapters/state is owned
// by user-service, not here - GET /api/v1/chapters is open to any authenticated caller, no extra
// permission needed). listAll() is used to resolve which chapters share a STATE_ADMIN's own state
// for EventAccessScope's state-tier - see EventServiceImpl.resolveChapterIdsInCallerState.
@Component
